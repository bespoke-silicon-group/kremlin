...
typedef struct _SEntry {
	uint32_t 	addrHigh;	// upper 32 bits in 64bit addr
	SegTable* segTable;
} SEntry;
...
Time* MShadowBase::get(Addr addr, Index size, Version* vArray, uint32_t width) {
...
void MShadowBase::set(Addr addr, Index size, Version* vArray, Time* tArray, uint32_t width) {
	if (size < 1)
		return;
